...
\begin{itemize}
  ...
  \item We propose \method, a domain-routed multi-LoRA-teacher on-policy distillation method that integrates standard-domain and real-world LoRA teachers into a single student, with PCBA-specific prompt canonicalization, input-format preservation, and metric-aligned answer verifiers.
...
The full training recipe keeps the standard-to-real transfer direction and folds the curriculum into verifier-aligned training. We first canonicalize the released samples into the shared prompt and short-answer protocol, preserving the original image structure, template-target pairs, question wording, options, family metadata, and numeric-answer format. We then train a shared PCBA base policy, specialize the standard-domain and real-world LoRA teachers from this base, freeze the teachers, and run mixed-domain on-policy distillation with domain-routed teacher prefill. The last stage is short verifier tuning on normalized answer spans, calibrating option scoring, defect-existence balance, numeric normalization, and answer-span likelihood under Eq.~\ref{eq:final_loss}. Thus the recipe uses the verifiers as the metric-aligned end of the same training pipeline rather than as a separate post-processing step.
...
The key design choice is to preserve the benchmark's two-domain asymmetry. Standards-derived items carry normative semantics about defect definitions, causes, and handling, while real images expose clutter, dense layouts, template comparisons, and exact counting requirements. Flat SFT collapses these signals into one average answer style, making it easy to memorize defect-policy associations without verifying the visual evidence. \method{} keeps the domains aligned through a shared prompt and answer protocol, but specializes one teacher per domain before distilling them into one student.
