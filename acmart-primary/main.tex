...
\label{eq:lora}
\end{equation}
where $A^{(d)}$ and $B^{(d)}$ are trainable low-rank matrices, $r$ is the adapter rank, and $\alpha$ is the LoRA scaling factor. The frozen teacher policy for domain $d$ is therefore $\pi_{\phi_d}=\pi_{\theta_0,\Delta_d}$, where $\Delta_d$ denotes the LoRA adapter. This design reduces the cost of building multiple teachers: the two teachers share the same backbone weights and differ only in small domain-specific adapters.
...
We evaluate on the PCBA Standard-to-Real Grand Challenge. The released public split contains 16,400 questions, with 8,200 training questions and 8,200 test questions~\cite{pcba_challenge_eval,pcba_challenge_dataset}. The dataset includes standards-derived VQA and real-world production-line VQA. The question families are standard-based knowledge QA, factuality, quantitative reasoning, and attribute reasoning. The official platform reports a normalized overall score using accuracy for single-choice questions, F1 for defect existence, and bounded normalized MAE for counting~\cite{pcba_challenge_eval}.
...
The framework is model-agnostic, but we use two Qwen3.5 model scales for different experimental purposes. All controlled ablations are conducted with Qwen3.5-9B, which keeps every variant under the same affordable compute budget and makes the effect of each method component easier to isolate. The final public-test submission scales the same recipe to Qwen3.5-27B. The two teachers are initialized from the same PCBA instruction-tuned checkpoint at the corresponding scale, trained as separate LoRA adapters, and frozen during \method{} distillation. Unless otherwise stated, the policy-gradient advantage is clipped to $A_{\max}=5$, and the final answer loss is applied only to normalized answer spans. The student is also trained parameter-efficiently in our implementation and can be merged with the base model for submission.
...
We compare \method{} with four baselines on Qwen3.5-9B. \emph{Base MLLM} evaluates the shared model before PCBA post-training. \emph{Flat SFT} trains on all public training data with the same prompt and answer format but without teacher specialization. \emph{Off-policy teacher SFT} trains on teacher-generated completions rather than student trajectories. \emph{LoRA teacher ensemble} routes prompts to the two domain LoRA teachers at inference time, testing whether explicit ensembling is sufficient without distilling teachers into a student. The most important comparison is between \method{} and off-policy teacher SFT: both use the same teachers, but only \method{} trains on student-generated trajectories.

\begin{table}[t]
\centering
\caption{Expected Qwen3.5-9B ablation results. Values other than the full \method{} score are planning placeholders for the controlled ablation runs.}
\vspace{-1.0em}
\label{tab:ablation}
\def\ablationscale{0.7} % Adjust this value to scale the table width.
\resizebox{\ablationscale\columnwidth}{!}{%
\begin{tabular}{lcc}
\toprule
Variant & Overall & S2R Gap \\
\midrule
\method{} & TBD & TBD \\
w/o standard-domain teacher & TBD & TBD \\
w/o real-world teacher & TBD & TBD \\
off-policy distillation & TBD & TBD \\
LoRA teacher ensemble & TBD & TBD \\
flat mixed SFT & TBD & TBD \\
\bottomrule
\end{tabular}
}
% \vspace{-0.7em}
...
The ablations are designed to test the core hypothesis that the challenge is best solved by structured cross-domain distillation. Removing the standard-domain teacher should hurt standard knowledge, cause, and handling questions because those answers depend on normative rules. Removing the real-world teacher should hurt real-image defect existence, defect type, mount side, attribute reasoning, template comparison, and counting. Off-policy distillation should trail \method{} when student generation diverges from static teacher completions, especially for questions requiring exact answer normalization.

We also report the standard-to-real gap, computed as the difference between standards-derived and real-world subsets when labels are available. A smaller gap indicates that the method transfers standards knowledge without becoming overconfident on visually different real images. Qualitative analysis will focus on three failure modes: visually plausible but standards-inconsistent handling decisions, correct defect type with wrong root cause, and counting errors caused by repeated small components or partially occluded pins.

\begin{table}[t]
\centering
\caption{Expected per-domain diagnostics for Qwen3.5-9B \method{}. Values are placeholders until controlled subset scoring is finalized.}
\vspace{-1.0em}
\label{tab:diagnostic}
\def\diagnosticscale{0.7} % Adjust this value to scale the table width.
\resizebox{\diagnosticscale\columnwidth}{!}{%
\begin{tabular}{lccc}
\toprule
Subset & Accuracy & Count MAE & Notes \\
\midrule
Standards-derived & TBD & -- & rule/action transfer \\
Real-world & TBD & -- & real-image perception \\
Template-target & TBD & TBD & missing/polarity matching \\
High-count cases & TBD & TBD & dense count errors \\
\bottomrule
\end{tabular}
}
% \vspace{-0.7em}
