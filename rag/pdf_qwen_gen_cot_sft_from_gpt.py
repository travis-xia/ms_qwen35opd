#!/usr/bin/env python3
"""
用 vLLM 根据「RAG Top-K 页图文交错 + GPT 参考答案/证据」生成 CoT，并组装 ms-swift SFT jsonl。

数据规则（与线上一致）：
  - user：与 pdf_qwen_infer / pdf_qwen_test 相同（不含教师参考答案）。
  - assistant：<think> 由模型生成；<answer>/<evidence> 来自 GPT CSV。
  - answer 经 normalize_submission_answer 与推理提交格式对齐。
  - evidence：ref∩RAG 可见页；交集为空则用 GPT 标签中的完整证据页（与答案一并记忆）。
  - OUT_RAW_JSON：含 question、gpt_answer（CSV 原文）、gpt_answer_norm、gpt_evidence_pages 等。

依赖：transformers、torch、vllm；在 rag 目录下执行。

示例:
  cd rag
  MODEL_PATH=... OUTPUT_TEST_DIR=... GPT_CSV=gpt.csv \\
  python3 pdf_qwen_gen_cot_sft_from_gpt.py

  # 四卡张量并行（未设置时默认使用全部可见 GPU）
  TENSOR_PARALLEL_SIZE=4 python3 pdf_qwen_gen_cot_sft_from_gpt.py

  # 关闭超长处理（重试 + 提炼）
  COT_REFINE_ENABLED=0 python3 pdf_qwen_gen_cot_sft_from_gpt.py

  # 过短/过长：各重生成 3 次（过短取最长且保留，过长取最短合格稿），仍超长再提炼
  COT_RETRY_SAMPLES=3 COT_REFINE_TOKEN_THRESHOLD=2048 python3 pdf_qwen_gen_cot_sft_from_gpt.py
"""
...
GPT_CSV = os.environ.get("GPT_CSV", "gpt.csv")
OUT_JSONL = os.environ.get("OUT_JSONL", "pdf_rag_gpt_cot_sft.jsonl")
OUT_RAW_JSON = os.environ.get("OUT_RAW_JSON", "pdf_rag_gpt_cot_raw.json")
OUT_SKIPPED_JSONL = os.environ.get("OUT_SKIPPED_JSONL", "pdf_rag_gpt_cot_skipped.jsonl")
OUT_MISMATCH_REPORT = os.environ.get(
    "OUT_MISMATCH_REPORT", "evidence_mismatch_report.json"
)
OUT_LENGTH_STATS = os.environ.get(
    "OUT_LENGTH_STATS", "pdf_rag_gpt_cot_length_stats.json"
)
PAGE_CACHE_DIR = os.environ.get("PAGE_CACHE_DIR", "pdf_rag_gpt_cot_page_cache")
...
def load_gpt_by_id(csv_path: str) -> Dict[str, Dict[str, str]]:
...
def load_merged_rows(script_dir: str) -> List[Dict[str, Any]]:
    ...
    gpt_path = _resolve(GPT_CSV, script_dir)
    rag_by_id = load_rag_by_id(rag_path)
    gpt_by_id = load_gpt_by_id(gpt_path)

    if not os.path.isfile(csv_path):
        raise FileNotFoundError(f"未找到题目 CSV: {csv_path}")
    if not os.path.isfile(gpt_path):
        raise FileNotFoundError(f"未找到 GPT 答案 CSV: {gpt_path}")
    ...
    for cr in csv_rows:
        ...
        if qid not in gpt_by_id:
            raise KeyError(f"{GPT_CSV} 缺少 id={qid}")
        rag = rag_by_id[qid]
        gpt = gpt_by_id[qid]
        ...
        ref_evidence_full = parse_evidence_column(gpt["evidence_page_number"])
        ...
        norm_answer = normalize_submission_answer(
            gpt["answer"], answer_format, language
        )
        rows.append(
            {
                "id": qid,
...
def main() -> None:
    ...
    print(f"GPT_CSV={_resolve(GPT_CSV, script_dir)}")
    ...
    with open(out_jsonl, "w", encoding="utf-8") as fout, open(
        ...
        for item in gen_results:
            ...
            raw_rec = {
                ...
                "gpt_answer": row.get("ref_answer_raw"),
                "gpt_answer_norm": row["ref_answer"],
                "gpt_evidence_pages": row["ref_evidence_full"],
